...
List reports command previously only listed enabled reports. Now the option list all reports together with its
enabled/disabled status. Also, internal reports are now omitted::
    ...
    Enable report by passing report name using --reports option. Use `all` to enable all default reports.
...
Same as ``--list`` rules option, ``--list-reports`` now support optional ``ENABLED`` / ``DISABLED`` argument to filter
