...
Severity threshold print value (#791)
-------------------------------------

Rules with severity threshold should now print the message with the
severity threshold value instead of default value::
...
Previously, if the severity threshold was not configured the rule was reported for default value.
Now not configured severity threshold are omitted::

    robocop -c line-too-long:severity_threshold:error=150

With above configuration, ``line-too-long`` will be reported only with error severity.
