...
However it was confusing to see reports for the same rule with different severities but the same limit value.
...
For example, if the ``line-too-long`` (with default severity W) is configured to have severity E if number of lines
is higher than 150::

    robocop -c line-too-long:severity_threshold:error=150

With above configuration, ``line-too-long`` will only be reported if number of lines is higher than 150.
Rule will work as before if all rule severity thresholds are configured::
