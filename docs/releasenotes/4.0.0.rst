...
``json_report`` was created from the old, internal only report with the same name. Previous report is renamed to
``internal_json_report``. If you are using Robocop programmatically and you are using``json_report``, rename it to
