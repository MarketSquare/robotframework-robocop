...
Added several new rules, new reports (including comparison with the previous run!). Our rule disablers are smarter
(context aware) and new rules can be defined to be disabled by default. Checkers & rules importing was also rewritten to
allow to load external rules from the modules imported inside Python file.

Our documentation was also updated with better formatting of the rules.

There are backward-incompatible changes to rule severity thresholds and rule disablers (described below in details).
