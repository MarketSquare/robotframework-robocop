...
``ReplaceRunKeywordIf`` is replaced by the fix for the ``deprecated-run-keyword-if`` (DEPR08) rule.
``NormalizeAssignments`` is replaced by the fixes for the ``inconsistent-assignment`` (MISC04) and
``inconsistent-assignment-in-variables`` (MISC05) rules.
``DiscardEmptySections`` is replaced by the fix for the ``empty-section`` (LEN09) rule. Note that, unlike the
