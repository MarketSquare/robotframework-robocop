"""
Shared logic for generating the ``rules_list.md`` documentation page.

The rules list is rendered from the live rule metadata (docstrings, parameters, messages) so it always
matches the installed Robocop version. This module is imported both by:

- ``docs/generate_rules_list.py`` - the mkdocs ``gen-files`` hook that renders the page for the site build.
- ``docs/update_rules_list.py`` - the standalone script that writes/verifies the committed ``docs/rules_list.md``
  file (used by pre-commit and CI so the committed copy stays in sync, e.g. for Context7 indexing).

It intentionally has no dependency on ``mkdocs`` so it can run in a plain environment.
"""
...
{% if rule_doc.deprecated_names %}
Deprecated names: {{ rule_doc.deprecated_names|join(", ") }}
...
def get_checker_docs() -> tuple[list, int]:
...
def generate_rules_list() -> str:
    """Render the full ``rules_list.md`` content from live rule metadata."""
    template = Template(RULES_LIST_TEMPLATE, trim_blocks=True, lstrip_blocks=True)
