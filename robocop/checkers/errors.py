...
rules = {
    ...
    "0405": Rule(
        ...
        msg="Invalid continuation mark '%s'. It should be '...'",
        severity=RuleSeverity.ERROR,
    ),
    ...
    "0410": Rule(
        ...
        msg="Provide at least two spaces after '%s' variable name",
        severity=RuleSeverity.ERROR,
    ),
    "0411": Rule(
        rule_id="0411",
...
class ParsingErrorChecker(VisitorChecker):
    ...
    def handle_invalid_continuation_mark(self, node, name):
        ...
        if len(stripped) == 2 or not stripped[2].strip():
            self.report("invalid-continuation-mark", stripped, node=node, col=name.find(".") + 1)
        elif len(stripped) >= 4:
            if stripped[:4] == "....":
                self.report("invalid-continuation-mark", stripped, node=node, col=name.find(".") + 1)
            else:  # '... ' or '...value' or '...\t'
