...
class LengthChecker(VisitorChecker):
    ...
    def visit_Keyword(self, node):  # noqa
        ...
        if key_calls < self.keyword_min_calls:
            self.report("too-few-calls-in-keyword", key_calls, self.keyword_min_calls, node=node)
            return
    ...
    def visit_TestCase(self, node):  # noqa
        ...
        if length > self.testcase_max_len:
            self.report("too-long-test-case", length, self.testcase_max_len, node=node)
        key_calls = LengthChecker.count_keyword_calls(node)
...
class EmptySectionChecker(VisitorChecker):
    ...
    rules = {"0509": ("empty-section", "Section is empty", RuleSeverity.WARNING)}
...
class EmptySettingsChecker(VisitorChecker):
    ...
    rules = {
        ...
        "0512": ("empty-documentation", "Documentation is empty", RuleSeverity.WARNING),
        "0513": ("empty-force-tags", "Force Tags are empty", RuleSeverity.WARNING),
        "0514": ("empty-default-tags", "Default Tags are empty", RuleSeverity.WARNING),
        "0515": (
        ...
        "0518": ("empty-setup", "Setup does not have any keywords", RuleSeverity.ERROR),
        "0519": (
            "empty-suite-setup",
        ...
        "0524": ("empty-timeout", "Timeout is empty", RuleSeverity.WARNING),
        "0525": ("empty-test-timeout", "Test Timeout is empty", RuleSeverity.WARNING),
        "0526": ("empty-arguments", "Arguments are empty", RuleSeverity.ERROR),
    }
