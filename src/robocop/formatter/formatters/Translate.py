...
if TYPE_CHECKING:
    from robot.parsing.model.blocks import (
        CommentSection as CommentSectionBlock,
    )
    from robot.parsing.model.blocks import (
        File,
        KeywordSection,
    ...
    *** Testfälle ***
    ...
    def __init__(
        ...
        if Language is not None:
            ...
            self.settings: dict[str, str] | None = {
                value: key.title() for key, value in self.language.settings.items() if key
            }
        else:
            self.language = None
            self.settings = None
    ...
    def bdd_mapping(self) -> dict[str, str]:
        if self._bdd_mapping is None:
            self._bdd_mapping = {}
            for language in self.languages:  # type: ignore[union-attr]
    ...
    def visit_KeywordCall(self, node: KeywordCall) -> KeywordCall:  # noqa: N802
        ...
        if not name or prefix.title() not in self.languages.bdd_prefixes:  # type: ignore[union-attr]
            return node
    ...
    def translate_section_header(
        self,
        node: TestCaseSection | KeywordSection | VariableSection | SettingSection | CommentSectionBlock,
        eng_name: str,
    ) -> TestCaseSection | KeywordSection | VariableSection | SettingSection | CommentSectionBlock:
    ...
    def visit_SettingSection(self, node: SettingSection) -> SettingSection:  # noqa: N802
        ...
        result = self.translate_section_header(node, "settings_header")
        self.in_settings = False
        return result
    ...
    @skip_section_if_disabled
    def visit_CommentSection(self, node: CommentSectionBlock) -> CommentSectionBlock:  # noqa: N802
        if node.header is None:
            return node
        return self.translate_section_header(node, "comments_header")

    @skip_if_disabled
    def visit_ForceTags(self, node: ForceTags) -> ForceTags:  # noqa: N802
        ...
        english_value = self.languages.settings.get(node_type, None)  # type: ignore[union-attr]
        if english_value is None:
            return node
        translated_value = self.settings.get(english_value, None) if self.settings else None
    ...
    def visit_Setup(self, node: Setup) -> Setup:  # noqa: N802
        ...
        translated_value = self.settings.get(node_type, None) if self.settings else None
        if translated_value is None:
            return node
