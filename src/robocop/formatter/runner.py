...
class RobocopFormatter:
    ...
    def format_until_stable(
        self, model: File
    ) -> tuple[bool, misc.StatementLinesCollector | None, misc.StatementLinesCollector | None, File]:
    ...
    def print_to_stdout(self, collected_lines: misc.StatementLinesCollector) -> None:
    ...
    def save_model(self, source: Path, model: File) -> None:
        if self.config.formatter.overwrite_files:
            output_path = str(self.config.formatter.output or source)
            misc.ModelWriter(output=output_path, newline=self.get_line_ending(str(source))).write(model)
        ...
        if self.config.formatter.whitespace_config.line_ending == "auto":
            with open(path) as f:
                ...
                if isinstance(f.newlines, str):
                    return f.newlines
                return f.newlines[0]
        return self.config.formatter.whitespace_config.line_ending or os.linesep

    def output_diff(
        self, path: Path, old_model: misc.StatementLinesCollector, new_model: misc.StatementLinesCollector
    ) -> None:
        if not self.config.formatter.diff:
            return
