...
from robocop.config import Config, ConfigManager  # noqa: TC001
from robocop.exceptions import ConfigurationError
from robocop.files import get_relative_path
from robocop.linter import sonar_qube
from robocop.linter.diagnostics import Diagnostic, Diagnostics  # noqa: TC001
    ...
    **Report name**: ``sonarqube``
    ...
    def generate_report(  # type: ignore[override]
        ...
        **kwargs: object,  # noqa: ARG002
        ...
        report = self.report_generator().generate_sonarqube_report(diagnostics, Path(config_manager.root))
...
class SonarQubeGenerator:
    ...
    def generate_sonarqube_report(self, diagnostics: Diagnostics, root: Path) -> dict[str, Any]:
        report: dict[str, Any] = {"rules": [], "issues": []}
...
class SonarQubeDescriptor99(SonarQubeGenerator):
    ...
    def get_code_attributes(self, rule: Rule) -> dict[str, Any]:
        if rule.sonar_qube_attrs:
            ...
            if rule.sonar_qube_attrs.issue_type == sonar_qube.SonarQubeIssueType.BUG:
                severity = "BLOCKER"
            else:
