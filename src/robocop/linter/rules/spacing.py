...
if TYPE_CHECKING:
    ...
    from robot.api.parsing import Token as TokenType
...
class VariableNotLeftAlignedRule(Rule):
    ...
    deprecated_names = ("1014", "variable-should-be-left-aligned")  # type: ignore[assignment]
...
class SuiteSettingNotLeftAlignedRule(Rule):
    ...
    deprecated_names = ("1016", "suite-setting-should-be-left-aligned")  # type: ignore[assignment]
...
class InconsistentUseOfTabsAndSpacesChecker(VisitorChecker):  # TODO: add found tab in file rule (to list them all)
    ...
    def __init__(self) -> None:
        self.tabs: list[TokenType] = []
        self.spaces: list[TokenType] = []
        super().__init__()
...
class MisalignedContinuation(VisitorChecker):
    ...
    @staticmethod
    def get_indent(tokens: list[TokenType]) -> int:
        indent_len = 0
        for token in tokens:
            if token.type != Token.SEPARATOR:
                break
    ...
    @staticmethod
    def first_line_indent(tokens: list[TokenType], from_tok: str, search_for: str) -> int:
        """
...
class LeftAlignedChecker(VisitorChecker):
    ...
    def parse_error(self, node: Node, error: str) -> None:
        setting_error_match = re.search("Non-existing setting '(.*)'.", error)
        if not setting_error_match:
            return
        setting_error = setting_error_match.group(1)
