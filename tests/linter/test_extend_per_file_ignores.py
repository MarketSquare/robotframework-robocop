...
def test_per_file_ignores():
    ...
    for diag in diagnostics:
        if diag.source.path.match("test.robot"):
            assert diag.rule.rule_id != "VAR02"
        if diag.source.path.match("ignore_subdir/*"):
            assert diag.rule.rule_id not in {"DOC01", "SPC09"}
        if diag.source.path.match("ignore_only_here/test2.robot"):
...
def test_per_file_ignores_disable_config():
    ...
    assert any(
        diag.rule.rule_id == "SPC10" for diag in diagnostics if diag.source.path.match("ignore_only_here/test2.robot")
    )
